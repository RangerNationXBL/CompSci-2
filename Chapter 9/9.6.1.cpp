/*Implement a class that simulates a traffic light. The next function advances the color in the 
usual way, from green to yellow to red, then again to green. Provide two constructors, as described 
in the documentation of the public interface. Also supply a member function that yields the number 
of times that this traffic light has been red.*/
...
TrafficLight::TrafficLight() : color("green"), reds(0)
{

}

TrafficLight::TrafficLight(string initial_color) : color(initial_color)
{
   if(color == "red"){reds++;}
}
