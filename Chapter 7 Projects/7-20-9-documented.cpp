...
/**
 * @brief Adds a student to the list of students
 * 
...
void addStudent(std::vector<std::unique_ptr<Student> > & students) {
    ...
    std::cout << "Please enter the student information (seperated by space)" << std::endl;
    std::cin >> newStudent->firstName >> newStudent->lastName; // >> newStudent->grade;

    //expand
    std::cout << "Please enter the quiz scores (seperated by space)" << std::endl;
    newStudent->quizScores.resize(10);
    
    // loops throgh the quiz scores that are entered by the user and stores them in the vecto.
    for(int i = 0; i < 10; i++){
        std::cin >> newStudent->quizScores[i];
    }

    // This loop calculates the sum of the quiz scores
    int totalScore = 0;
